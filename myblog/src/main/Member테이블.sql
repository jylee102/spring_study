...
SELECT * FROM (
	SELECT ROWNUM rnum, DATA.* FROM
		...
		WHERE p.CATEGORY_CODE = c.CATEGORY_CODE
		AND p.MEMBER_ID = 1 --특정 사용자의 게시물
		AND SUBJECT LIKE '%세번째%' --검색어
		ORDER BY p.POST_ID DESC) DATA --rownumber(행번호)
) WHERE rnum >= 1 AND rnum <= 3; --페이지

--getDataCount(페이징)
SELECT NVL(COUNT(*), 0) FROM POST WHERE MEMBER_ID = 1 AND SUBJECT LIKE '%%';
...
--updatePost
UPDATE POST SET SUBJECT = '수정한 제목', CONTENT = '수정한 내용', UPDATE_DATE = SYSDATE, CATEGORY_CODE = '002' WHERE POST_ID = 45;