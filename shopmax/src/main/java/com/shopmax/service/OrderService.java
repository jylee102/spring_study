...
public class OrderService {
    ...
    public Long order(OrderDto orderDto, String email) {
        ...
        // 양방향일 때에는 부모 엔티티 객체가 참조하는 자식 객체를 넣은 후 save
        // 단방향일 때에는 자식 엔티티 객체가 참조하는 부모 객체를 넣은 후 save
    ...
    public Page<OrderHistDto> getOrderList(String email, Pageable pageable) {
        ...
        // 주문 내역을 여러 개이므로 리스트에 저장
        List<OrderHistDto> orderHistDtos = new ArrayList<>();
        
        // 3. 주문 리스트 orders를 순회하면서 컨트롤러에 전달할 Dto(OrderHistDto)를 생성
        for (Order order : orders) {
            OrderHistDto orderHistDto = new OrderHistDto(order);

            // 양방향 참조를 하고 있으므로 order를 통해 orderItem을 가져온다.
            // JPA의 특성상 join을 하지 않아도 order 안에 orderItem 레코드가 들어있다.
            List<OrderItem> orderItems = order.getOrderItems();

            // ★ 최종적으로 orderItems 리스트를
            // orderHistDto에 있는 orderItemDtoList에 dto로 넣어줘야 한다.
